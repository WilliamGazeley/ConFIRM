...
def train(config=None):
    with wandb.init(config=config):
        config=wandb.config
        peft_model_id = f"ocean_masked_prefix_b{config.batch_size}_e{config.epochs}_lr{str(config.lr)}_maxl{config.max_length}_nvt{config.num_virtual_tokens}"
        print(peft_model_id)
